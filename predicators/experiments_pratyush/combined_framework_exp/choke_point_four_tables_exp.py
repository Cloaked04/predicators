...
table_workspaces: List[Dict[str, float]] = [
...
env = PyBulletMultiTableBlocksEnv(use_gui=False, num_tables=4,
								  table_poses=table_poses, 
								  table_workspaces=table_workspaces)
...
while plan_count<100:
    ...
    # Block from table 2      
    block_to_move2 = Object("block2_0_0", env._block_type)
    # Block from table 3      
    block_to_move3 = Object("block3_0_0", env._block_type)
    target_table1 = env._tables[1]
    target_table2 = env._tables[2]
    target_table3 = env._tables[3]
    target_table0 = env._tables[0] 
    ...
    goal = {  
        GroundAtom(env._OnTable, [block_to_move2, target_table2]),
        GroundAtom(env._OnTable, [block_to_move3, target_table2]),
        GroundAtom(env._OnTable, [block_to_move1, target_table0])
    }
